...
def connect_to_cassandra():
    try:
        ...
        print("ATTENTION: The Cassandra driver could not be imported.")
        print("Please ensure Cassandra-driver installed type: python -m pip install cassandra-driver into command prompt to verify.")
        return None

    cluster = None
    try:
        # Notifying when tconnectiond starting.
        print("Connecting to local Cassandra database...")

        # Connect to the local Cassandra cluster using the default settings.
        cluster = Cluster()
        session = cluster.connect()

        # Create keyspace
        keyspace = """
            CREATE KEYSPACE IF NOT EXISTS group2_github_archive
            WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1}
        """
        session.execute(keyspace)
        session.set_keyspace("group2_github_archive")

        # Creating table for each supported archive dataset.
        for dataset in DATASETS:
            table_name = cassandra_table_name(dataset)
            new_table = (
                "CREATE TABLE IF NOT EXISTS " + table_name + " ("
                "record_id text PRIMARY KEY, record_json text)")
            session.execute(new_table)

        # Fetch results in pages default max 100 rows when looping through records.
        session.default_fetch_size = 100
        print("Cassandra connection successful.")
        print("Keyspace: group2_github_archive")
        return session

    except (DriverException, NoHostAvailable, OSError, ValueError) as error:
        print("Unable to connect to Cassandra or create its tables:", error)
        if cluster is not None:
            cluster.shutdown()
        return None


def cassandra_table_name(dataset):
    # Only allowinbg names from existing dataset list.
    if dataset not in DATASETS:
        raise ValueError("Please choose one of the listed datasets.")
    return dataset.lower()


def cassandra_get_record(database, dataset, record_id):
    table_name = cassandra_table_name(dataset)
    query = database.prepare(
        "SELECT record_json FROM " + table_name + " WHERE record_id = ?")
    saved_row = database.execute(query, (record_id,)).one()
    if saved_row is None:
        return None
    return json.loads(saved_row.record_json)


def choose_cassandra_record(database):
    dataset, action = choose_dataset()
    if action != "continue":
        return None, None, None, action
    record_id, action = get_user_input("Please paste or enter the full Record ID. Use Search option if in need of reference.")
    if action != "continue":
        return None, None, None, action
    record = cassandra_get_record(database, dataset, record_id)
    return dataset, record_id, record, "continue"


def cassandra_import_archive(database):
    file_path, action = get_user_input("Please enter the path to the desired extracted JSON file")
    if action != "continue":
        return action
    dataset = os.path.splitext(os.path.basename(file_path))[0]
    if dataset not in DATASETS:
    ...
    while True:
        limit_text, action = get_user_input("Max records to read [100]")
        if action != "continue":
            return action
        if limit_text == "":
            maximum_records = 100
    ...
    table_name = cassandra_table_name(dataset)
    # Preparing once before loop, then supplies  each record's values with ? markers.
    #Cassandra INSERT is capable of overwriting an existing row. IF NOT EXISTS is being used to prevent this and count the dupes but keep in mind it seems to take some more time in comparison
    query = database.prepare(
        "INSERT INTO " + table_name + " (record_id, record_json) "
    ...
    for record in records:
        record_id = make_record_key(record, dataset)
        if record_id is None:
            invalid_records += 1
            continue
        result = database.execute(query, (record_id, json.dumps(record)))
...
def cassandra_create_record(database):
    new_record, dataset, action = collect_new_record()
    if action != "continue":
        return action
    record_id = make_record_key(new_record, dataset)
    if record_id is None:
        print("The identifying fields cannot be empty.")
        return option_finished()
    ...
    if confirmation.upper() == "Y":
        ...
        query = database.prepare(
            "INSERT INTO " + table_name + " (record_id, record_json) "
            "VALUES (?, ?) IF NOT EXISTS")
        result = database.execute(query, (record_id, json.dumps(new_record)))
        if result.was_applied:
            print("Record created. Record ID:", record_id)
        else:
            print("ATTENTION: A record with that ID already exists.")
    else:
        print("Create canceled.")
    return option_finished()


def cassandra_read_record(database):
    dataset, record_id, record, action = choose_cassandra_record(database)
    if action != "continue":
        return action
    if record is None:
        print("ATTENTION: Record not found in that dataset.")
    else:
...
def cassandra_update_record(database):
    dataset, record_id, record, action = choose_cassandra_record(database)
    if action != "continue":
        return action
    if record is None:
        print("ATTENTION: Record not found.")
        return option_finished()
    print(json.dumps(record, indent=2))
    ...
    if action != "continue":
        return action
    if field in KEY_FIELDS[dataset]:
        print("ATTENTION: The Identifying fields cannot be changed. Create a new record instead.")
        return option_finished()
    if field not in record:
        print("Please choose an existing (top-level) field shown above.")
        return option_finished()

    value, action = get_user_input("Please enter new value (use JSON for lists/objects/booleans)")
    if action != "continue":
        return action
    try:
        record[field] = convert_value(value, record[field])
    except ValueError as error:
        print("Invalid value:", error)
        return option_finished()

    table_name = cassandra_table_name(dataset)
    # Edit the dictionary and save the complete JSON, like the Redis section.
    # IF EXISTS stops an update from creating a missing row.
    query = database.prepare(
        "UPDATE " + table_name + " SET record_json = ? "
        "WHERE record_id = ? IF EXISTS")
    result = database.execute(query, (json.dumps(record), record_id))
    if result.was_applied:
...
def cassandra_delete_record(database):
    dataset, record_id, record, action = choose_cassandra_record(database)
    if action != "continue":
        return action
    if record is None:
    ...
    if confirmation.upper() == "DELETE":
        table_name = cassandra_table_name(dataset)
        query = database.prepare(
            "DELETE FROM " + table_name + " WHERE record_id = ? IF EXISTS")
        result = database.execute(query, (record_id,))
        if result.was_applied:
...
#feat 1: search JSON records and choose a match to view can also be useful for finding desired record for other uses.
def cassandra_search_records(database):
    dataset, action = choose_dataset()
    if action != "continue":
        return action
    search_word, action = get_user_input(
        "Please enter a word, path, hash, or key")
    if action != "continue":
        return action

    table_name = cassandra_table_name(dataset)
    print("Searching records in", dataset)
    rows = database.execute("SELECT record_id, record_json FROM " + table_name)
    matching_records = []
    for row in rows:
        if search_word.lower() in (row.record_id + " " + row.record_json).lower():
            matching_records.append(row.record_id)
            print(len(matching_records), "Record ID:", row.record_id)
            if len(matching_records) == 25:
                print("Showing up to 25 matches. Please narrow the search for a closer result.")
                break
    ...
    if len(matching_records) == 0:
        ...
        return option_finished()
    while True:
        selection, action = get_user_input("Please enter a result number to view")
        if action != "continue":
            return action
        if selection.isdigit() and 1 <= int(selection) <= len(matching_records):
            record_id = matching_records[int(selection) - 1]
            record = cassandra_get_record(database, dataset, record_id)
            print("Record ID:", record_id)
            if record is None:
                print("This record no longer exists.")
                ...
                print(json.dumps(record, indent=2))
            return option_finished()
        print("Please choose a result number from the list to continue.")


#feat 2: counts groups, ex Licenses grouped by license can be useful for scanning through data or est. a quick log of data to reference for other options.
def cassandra_group_records(database):
    dataset, action = choose_dataset()
    if action != "continue":
        return action
    field, action = get_user_input("Enter a top-level field such as license, repo_name, or mode")
    if action != "continue":
        return action
    if field == "":
    ...
    table_name = cassandra_table_name(dataset)
    rows = database.execute("SELECT record_json FROM " + table_name)
    grouped_records = {}
    for row in rows:
        record = json.loads(row.record_json)
        # Like the existing grouping functions, a whole list is one group value.
        group_value = str(record.get(field, "Field not found"))
        if group_value not in grouped_records:
            grouped_records[group_value] = 1
        else:
            grouped_records[group_value] += 1
    ...
    if len(grouped_records) == 0:
        ...
        print(f"Records grouped by '{field}':")
        for group_value in grouped_records:
            print(group_value, ":", grouped_records[group_value])
    return option_finished()


#Feat 3: exports selected datasets (one JSON record per line).
def cassandra_export_records(database):
    dataset, action = choose_dataset()
    if action != "continue":
        return action
    output_file, action = get_user_input("Enter an output JSON filename")
    if action != "continue":
        return action
    if os.path.exists(output_file):
        confirmation, action = get_user_input("Type OVERWRITE to replace the existing file")
        if action != "continue":
            return action
        if confirmation != "OVERWRITE":
            print("Export canceled.")
    ...
    with open(output_file, "w", encoding="utf-8") as output:
        for row in rows:
            output.write(row.record_json + "\n")
            records_written += 1
    print(records_written, "records exported.")
    print("Please select", dataset, "when importing this file again.")
    return option_finished()


def cassandra_help():
    print("\nCASSANDRA FAQ AND HELP")
    print("If you are having an issue where cassandra import is not running at all when choosing this database option please make the driver is installedd: python -m pip install cassandra-driver in command prompt.")
    print("The keyspace is group2_github_archive. Table names are lowercase datasets.")
    print("Extract the archive ZIP, then import one JSON file with option 1.")
    print("Use the Search option to copy a full Record ID you are seeking for Reading, Updating, or Deleting.")
    print('Example ID template resembles this: github:archive:Sample_Repos:["owner/repo"]')
    print("Option 6 lets user search the selected dataset's keys and JSON text.")
    print("Option 7 example: choose Licenses, then enter the license to count each license.")
    print("Option 8 exports one JSON record per line to a file.")
    print("Search and grouping scan records; this program responds better with small import amounts.")
    print("At data and Help prompts, B will return user to the Cassandra menu and E will exit the program.")
    return option_finished()


def cassandra_menu():
    database = connect_to_cassandra()
    if database is None:
        print("Attention: An issue has occured when attempting to connect to Cassandra Database please check that the Cassandra-driver installation was completed and check the Cassandra server, then re-attempt.")
        return
    from cassandra import DriverException
    ...
    try:
        while app_cont:
            display_menu()
            print("Database: Cassandra")
            print("9) Cassandra Help")
            choice = input("Please select an option: ").strip()
            result = None
            try:
                if choice == "1":
                    result = cassandra_import_archive(database)
                elif choice == "2":
                    result = cassandra_create_record(database)
                elif choice == "3":
                    result = cassandra_read_record(database)
                elif choice == "4":
                    result = cassandra_update_record(database)
                elif choice == "5":
                    result = cassandra_delete_record(database)
                elif choice == "6":
                    result = cassandra_search_records(database)
                elif choice == "7":
                    result = cassandra_group_records(database)
                elif choice == "8":
                    result = cassandra_export_records(database)
                elif choice == "9":
                    result = cassandra_help()
                elif choice == "0":
                    app_cont = False
                else:
                    print("Enter a number from 0 through 9.")
            except (DriverException, NoHostAvailable, OSError, ValueError, TypeError) as error:
                print("Operation has stopped:", error)
                print("Please be aware that some records or exported lines may already have been saved.")
                print("After a database has timed out, don't forget to read the record in order to check its current status.")
                result = option_finished()
            if result == "exit":
                app_cont = False
    finally:
